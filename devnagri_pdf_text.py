...
class Font:
	...
	def id_unicode_i_r(self, id, prkriya = False):
		if type(id) == list:
			idn = []
			i = 0
			while i < len(id):
				...
				idn.append(id[i])
				i = j
			return ''.join([self.id_unicode_i_r(i, prkriya) for i in idn])
		...
		if aksrnam in self.aksrnam_datvh.keys():
			...
			return self.id_unicode_i_r([self.aksrnam_id[datuh] for datuh in self.aksrnam_datvh[aksrnam]], prkriya)

		for adesh in self.adesah:
			if len(adesh[0]) == 1 and adesh[0][0] == aksrnam:
				if prkriya:
					print('आ॒दे॒शः ' + aksrnam + ' > ' + self.adesh_aksrnam[aksrnam])
				return self.id_unicode_i_r(self.aksrnam_id[adesh[1]])

		return ''
	
	def id_unicode(self, id):
		r = self.id_unicode_i_r(id)
